...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_CAPTURE_MODE</code></td><td>Enable capture recorder mode for stream input and direct in-browser user actions. Set to <code>on</code> to write structured JSONL capture events.</td><td>(disabled)</td></tr>
    <tr><td><code>AGENT_BROWSER_CAPTURE_FILE</code></td><td>Append capture JSONL events to this file path.</td><td>(none)</td></tr>
    <tr><td><code>AGENT_BROWSER_CAPTURE_STDOUT</code></td><td>Mirror capture JSONL events to stdout when set to <code>1</code>.</td><td>(disabled)</td></tr>
    <tr><td><code>AGENT_BROWSER_CAPTURE_CHECKPOINTS</code></td><td>Checkpoint strategy for capture mode. Use <code>aggressive</code> for frequent event-time ref checkpoints.</td><td>(none)</td></tr>
